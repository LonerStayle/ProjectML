...
from agents.dungeon.monster.models import (
    MonsterMetadata,
    StatData,
    RoomData,
    MonsterSpawnData,
...
class MonsterCompositionAgent:
    ...
    def __init__(
        self,
        heroine_stats: List[StatData],
        monster_db: Dict[int, MonsterMetadata],
        model_name: str = LLM.GPT4_1_NANO,
    ):
        self.heroine_stats = heroine_stats
    ...
    def _calculate_party_score(self) -> float:
        ...
        """
        if not self.heroine_stats:
            return 0.0
        total_score = sum(h.combat_score for h in self.heroine_stats)
        return total_score / len(self.heroine_stats)

    def _get_llm_strategy(self, party_score: float, floor: int) -> Dict:
        """
        [Brain] LLM에게 히로인 정보를 주고 밸런싱 전략(JSON)을 받아옴
        """
        hero = self.heroine_stats[0]  # 대표 히로인 (또는 평균값 사용 가능)
        ...
        try:
            ...
            final_prompt = self.prompt_manager.get_prompt(
                hero_summary=hero_summary, floor=floor
            )
    ...
    def _select_monsters(
        self, target_score: float, preferred_tags: List[str]
        ...
        if candidates:
            ...
            weighted_pool = sorted(
                self.monster_db.values(), key=lambda x: x.cost_point
            )[:3]
    ...
    def process_dungeon(
        self, dungeon_json: Dict[str, Any]
        ...
        for room_dict in raw_rooms:
            ...
            if room.room_type == 1 and not room.monsters:
                ...
                for _ in range(count):
                    ...
                    spawn_list.append(
                        MonsterSpawnData(monster_id=mob.monster_id, pos_x=gx, pos_y=gy)
                    )

                room.monsters = spawn_list

            # 보스방(Type 3)이고 몬스터가 비어있으면 -> 보스 몬스터 배치
            elif room.room_type == 3 and not room.monsters:
                spawn_list = []
                # 보스는 1마리만 배치
                # 몬스터 DB에서 가장 강한 몬스터 선택 (cost_point 기준)
                boss_monster = max(self.monster_db.values(), key=lambda m: m.cost_point)
                
                # 중앙 좌표에 배치
                gx, gy = 0.5, 0.5
                
                spawn_list.append(
                    MonsterSpawnData(monster_id=boss_monster.monster_id, pos_x=gx, pos_y=gy)
                )
        ...
        for room in updated_rooms:
            if room.get("monsters"):
                total_monsters += len(room["monsters"])
